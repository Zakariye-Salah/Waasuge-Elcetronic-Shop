...
function showDashboardSkeleton() {
  ["totalProducts","totalRevenue","totalProfit","totalQuantity","importantProducts","lowStockProducts","totalInvoices","totalRepairs","pendingRepairs","processingRepairs","completedRepairs","deliveredRepairs","totalRemainingMoney","repairPaidTotal","repairUnpaidTotal","totalExpense","netProfit"].forEach((id) => {
    const el = document.getElementById(id);
    if (el) {
...
function updateDashboardNotificationBadge({ products = [], invoices = [], repairs = [] } = {}) {
  const lowStock = filterActive(products).filter((item) => safeNumber(item?.quantity) <= LOW_STOCK_THRESHOLD).length;
...
function badgeClassForStock(quantity) {
  ...
  if (qty <= LOW_STOCK_THRESHOLD) return "bg-soft-warning text-warning-soft";
...
function renderLowStock(products) {
  ...
  const list = sortByDate(filterActive(products), "createdAt", true)
    .filter((item) => safeNumber(item?.quantity) <= LOW_STOCK_THRESHOLD)
    .slice(0, 6);
  if (!list.length) {
...
async function loadDashboardSummary() {
  ...
  showDashboardSkeleton();
  try {
    const [productsData, invoicesData, repairsData, expensesData] = await Promise.all([
