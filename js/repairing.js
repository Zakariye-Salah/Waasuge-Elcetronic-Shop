...
function buildRepairCustomerDirectory() {
  const records = [...getActiveRecords(state.repairs), ...getActiveRecords(state.invoices)];
  const map = new Map();
  records.forEach((record) => {
    const customerName = String(record?.customerName || record?.customer || record?.clientName || "").trim();
    const phone = String(record?.customerPhone || record?.phone || record?.customerPhoneNumber || record?.phoneNumber || "").trim();
    const key = normalizeRepairCustomerKey(record);
    if (!key || !customerName) return;
    const current = map.get(key) || { customerName, phone: phone || "", count: 0, lastActivity: 0 };
...
function buildCustomerContactMessage(summary) {
  return [
    `Waasuge Electronics & Mobile Repairing`,
    ...
    <div class="tracking-premium-shell">
        ...
        <div class="tracking-brand-top tracking-brand-top--compact">
          <div class="tracking-brand-main">
            ...
            <div class="tracking-brand-copy">
              <div class="text-uppercase small fw-bold text-primary mb-1">Waasuge Electronics and mobile repairing</div>
      ...
      <div class="tracking-footer mt-4">
        <div class="tracking-footer-copy">
          <div><i class="bi bi-shop me-2"></i>Waasuge Electronics and mobile repairing</div>
          <div><i class="bi bi-telephone me-2"></i>0617125558</div>
...
function syncRepairStatus() {
  ...
  let paid = Math.max(0, safeNumber(paidEl?.value, 0));
  if (paid > total) {
    paid = total;
    if (paidEl) paidEl.value = String(total);
...
function collectFormData() {
  ...
  const paidAmount = Math.max(0, Math.min(safeNumber(fieldValue("repairPaidAmount")), finalTotal));
  ...
  return {
    ...
    customerPhone: fieldValue("repairCustomerPhone").trim(),
  ...
  return `
    <div class="col" data-repair-id="${id}" data-search="${repairQueryBlob(repair)}">
      <div class="repair-card card-shell h-100 p-3">
        ...
        <div class="repair-action-grid d-flex flex-wrap gap-2 justify-content-end">
          <button class="btn btn-sm btn-outline-primary repair-action-btn action-btn" data-action="edit" data-id="${id}" title="Edit"><i class="bi bi-pencil-square"></i></button>
          <button class="btn btn-sm btn-outline-info repair-action-btn action-btn" data-action="history" data-id="${id}" title="Customer History"><i class="bi bi-clock-history"></i></button>
          <button class="btn btn-sm btn-outline-secondary repair-action-btn action-btn" data-action="tracking" data-id="${id}" title="Tracking"><i class="bi bi-stars"></i></button>
          <button class="btn btn-sm btn-outline-secondary repair-action-btn action-btn" data-action="status-next" data-id="${id}" title="Next status"><i class="bi bi-arrow-right-circle"></i></button>
          <button class="btn btn-sm btn-outline-${payment === "Paid" ? "warning" : "success"} repair-action-btn action-btn" data-action="payment-toggle" data-id="${id}" title="${payment === "Paid" ? "Mark unpaid" : "Mark paid"}"><i class="bi ${payment === "Paid" ? "bi-arrow-counterclockwise" : "bi-cash-coin"}"></i></button>
          <button class="btn btn-sm btn-outline-success repair-action-btn action-btn" data-action="whatsapp" data-id="${id}" title="WhatsApp"><i class="bi bi-whatsapp"></i></button>
          <button class="btn btn-sm btn-outline-secondary repair-action-btn action-btn" data-action="sms" data-id="${id}" title="SMS"><i class="bi bi-chat-dots"></i></button>
          <button class="btn btn-sm btn-outline-dark repair-action-btn action-btn" data-action="print" data-id="${id}" title="Print"><i class="bi bi-printer"></i></button>
          <button class="btn btn-sm btn-outline-danger repair-action-btn action-btn" data-action="delete" data-id="${id}" title="Delete"><i class="bi bi-trash3"></i></button>
        </div>
      </div>
    </div>
  `;
  ...
  return `
    <tr data-repair-id="${id}" data-search="${repairQueryBlob(repair)}">
      ...
      <td data-label="Actions">
        <div class="repair-action-grid d-flex flex-wrap gap-2">
          <button class="btn btn-sm btn-outline-primary action-btn" data-action="edit" data-id="${id}" title="Edit"><i class="bi bi-pencil-square"></i></button>
          <button class="btn btn-sm btn-outline-info action-btn" data-action="history" data-id="${id}" title="Customer History"><i class="bi bi-clock-history"></i></button>
          <button class="btn btn-sm btn-outline-secondary action-btn" data-action="tracking" data-id="${id}" title="Tracking"><i class="bi bi-stars"></i></button>
          <button class="btn btn-sm btn-outline-secondary action-btn" data-action="status-next" data-id="${id}" title="Next status"><i class="bi bi-arrow-right-circle"></i></button>
          <button class="btn btn-sm btn-outline-${paymentStatusLabel(repair) === "Paid" ? "warning" : "success"} action-btn" data-action="payment-toggle" data-id="${id}" title="${paymentStatusLabel(repair) === "Paid" ? "Mark unpaid" : "Mark paid"}"><i class="bi ${paymentStatusLabel(repair) === "Paid" ? "bi-arrow-counterclockwise" : "bi-cash-coin"}"></i></button>
          <button class="btn btn-sm btn-outline-dark action-btn" data-action="print" data-id="${id}" title="Print"><i class="bi bi-printer"></i></button>
          <button class="btn btn-sm btn-outline-success action-btn" data-action="whatsapp" data-id="${id}" title="WhatsApp"><i class="bi bi-whatsapp"></i></button>
          <button class="btn btn-sm btn-outline-info action-btn" data-action="sms" data-id="${id}" title="SMS"><i class="bi bi-chat-dots"></i></button>
          <button class="btn btn-sm btn-outline-danger action-btn" data-action="delete" data-id="${id}" title="Delete"><i class="bi bi-trash3"></i></button>
        </div>
      </td>
    </tr>
  `;
...
function renderRepairTableSkeleton(rows = 4) {
  return Array.from({ length: rows }, () => `
    <tr class="repair-skeleton-row"><td colspan="6"><div class="skeleton-line" style="width: 90%;"></div></td></tr>`).join('');
}
  ...
  try {
    if (editingId) {
      await updateRepair(editingId, {
        ...
        id: editingId,
        status: normalizeRepairStatus(getRepairByIdFromState(editingId)?.status || payload.status || "pending"),
        isDeleted: false,
        deleted: false,
        updatedAt: nowValue()
      });
      showToast("Repair job updated successfully", "success", "Repair");
    } else {
...
function fillForm(repair) {
  if (!repair) return;
  state.editId = repair.id || repair.repairId || null;
  setFieldValue("repairRecordId", state.editId || "");
  setFieldValue("repairCustomerName", repair.customerName || "");
  setFieldValue("repairCustomerPhone", repair.customerPhone || "");
  setFieldValue("repairDeviceName", repair.deviceName || "");
  setFieldValue("repairDeviceType", repair.deviceType || "");
  setFieldValue("repairProblem", getRepairProblemValue(repair) || "");
  setFieldValue("repairParts", getRepairPartsValue(repair) || (Array.isArray(repair.parts) ? repair.parts.join(", ") : ""));
  setFieldValue("repairPrice", String(repair.price ?? repair.total ?? 0));
  setFieldValue("repairDiscount", String(repair.discount ?? 0));
  setFieldValue("repairPaidAmount", String(repair.paidAmount ?? 0));
  setFieldValue("repairFinalTotal", String(repair.finalTotal ?? Math.max(safeNumber(repair.price ?? repair.total ?? 0) - safeNumber(repair.discount ?? 0), 0)));
  setFieldValue("repairStatus", displayStatusLabel(repair.status));
  setFieldValue("repairNotes", repair.notes || "");
  setFieldValue("repairDate", repair.repairDate || (repair.createdAt ? new Date(repair.createdAt).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10)));
  setFieldValue("repairWarrantyDays", repair.warrantyDays ?? 0);
  setFieldValue("repairWarrantyStart", repair.warrantyStart || repair.repairDate || (repair.createdAt ? new Date(repair.createdAt).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10)));
  setFieldValue("repairWarrantyExpiry", repair.warrantyExpiry || repair.warrantyEnd || repair.warrantyExpiresAt || "");
  setFieldValue("repairWarrantyStatus", repair.warrantyStatus || "No warranty");
  syncWarrantyFields();
  const submit = getEl("repairSubmitBtn");
  if (submit) submit.innerHTML = '<i class="bi bi-check2-circle me-1"></i> Update Repair';
}

function getRepairByIdFromState(id) {
  return state.repairs.find((item) => String(item.id || item.repairId) === String(id));
}

function buildShareText(repair) {
  const name = repair?.customerName || "Customer";
  const device = repair?.deviceName || "device";
  const status = statusMeta(repair?.status).label;
  const total = formatCurrency(repair?.finalTotal ?? repair?.price ?? 0);
  const paid = formatCurrency(repair?.paidAmount ?? 0);
  const balance = formatCurrency(repairBalance(repair));
...
function openShareLink(repair, kind) {
  const message = encodeURIComponent(buildShareText(repair));
  const phone = String(repair?.customerPhone || "").replace(/\D/g, "");

  if (kind === "whatsapp") {
    const url = `https://wa.me/${phone || "252000000000"}?text=${message}`;
    window.open(url, "_blank", "noopener,noreferrer");
    showToast("WhatsApp message ready for the customer", "success", "Repair");
    return;
  }
  ...
  if (kind === "sms") {
    ...
    showToast("Opening SMS app for the customer", "info", "Repair");
  }
}
...
function getRepairCustomerSuggestWrap() {
  return getEl("repairCustomerSuggestWrap");
}
...
function renderRepairCustomerSuggestions() {
  ...
  const matches = buildRepairCustomerDirectory()
    .filter((item) => !query || normalizeText(item.customerName).includes(query) || normalizeText(item.phone).includes(query))
    .slice(0, 8);
  ...
  panel.innerHTML = matches.map((item) => `
    <button type="button" class="customer-suggestion-item" data-customer-name="${escapeHtml(item.customerName)}" data-customer-phone="${escapeHtml(item.phone || "")}">
...
function applyRepairCustomerSuggestion(record) {
  const nameEl = getEl("repairCustomerName");
  const phoneEl = getEl("repairCustomerPhone");
  if (nameEl) nameEl.value = record?.customerName || "";
...
function bindRepairCustomerAutocomplete() {
  ...
  const syncFromName = () => {
    ...
    const exact = directory.find((item) => normalizeText(item.customerName) === query || (item.phone && normalizeText(item.phone) === query));
...
function buildCustomerHistoryBundle(seed = {}, invoices = [], repairs = []) {
  const key = normalizeRepairCustomerKey(seed);
  const invoiceRows = getActiveRecords(invoices)
    .filter((item) => normalizeRepairCustomerKey(item) === key)
  ...
  const combined = [...invoiceRows, ...repairRows].sort((a, b) => (b.stamp || 0) - (a.stamp || 0));
  const totals = [...invoiceRows, ...repairRows].reduce((acc, row) => {
...
function renderCustomerHistoryHtml(summary, rows) {
  const body = rows.length
    ...
    : `<tr><td colspan="5" class="text-center text-muted py-4">No customer history found.</td></tr>`;
    ...
    <div class="card-shell">
      ...
      <div class="section-body table-responsive">
        <table class="table align-middle mb-0">
          <thead><tr><th>Type</th><th>Reference</th><th>Details</th><th>Amount</th><th>Date</th></tr></thead>
          <tbody>${body}</tbody>
        </table>
      </div>
    </div>`;
...
async function openRepairCustomerHistory(repair) {
  ...
  try {
    ...
    const { summary, rows } = buildCustomerHistoryBundle(repair || {}, state.invoices, repairs);
...
function printRepair(repair) {
  ...
  const phone = escapeHtml(repair?.customerPhone || "");
  ...
  const total = escapeHtml(formatCurrency(repair?.finalTotal ?? repair?.price ?? 0));
  const paid = escapeHtml(formatCurrency(repair?.paidAmount ?? 0));
  const balance = escapeHtml(formatCurrency(Math.max(0, safeNumber(repair?.finalTotal ?? repair?.price ?? 0) - safeNumber(repair?.paidAmount ?? 0))));
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          .queue-text strong { display:block; font-size: 18px; margin-bottom: 2px; }
          .queue-text span { color: #475569; font-size: 13px; }
          .grid {
            display: grid;
          ...
          .pill.status { background: ${status?.softBg || '#eff6ff'}; color: ${status?.color || '#1d4ed8'}; }
          ...
          .details th { width: 34%; background: #f8fafc; color: #475569; font-size: 12px; text-transform: uppercase; letter-spacing: .05em; }
          .details td { font-size: 14px; color: #0f172a; font-weight: 600; }
          .summary {
            display: grid;
          ...
          .sum .k { display:block; font-size: 11px; text-transform: uppercase; letter-spacing: .08em; opacity: .8; margin-bottom: 4px; }
          .sum .v { display:block; font-size: 18px; font-weight: 800; }
          .footer {
            margin-top: 16px;
            display: flex;
            justify-content: space-between;
      ...
      <body>
        <div class="sheet">
          <div class="header">
            <div class="brand">
              <div>
                <h1>Waasuge Electronics & Mobile Repairing</h1>
                <p>Premium repair ticket • professional service receipt</p>
              </div>
              <div class="meta">
                <div><strong>Printed:</strong> ${printedDate}</div>
                <div><strong>Repair ID:</strong> ${repairNo}</div>
                <div><strong>Queue No.:</strong> ${queueNo}</div>
              </div>
            </div>
          </div>
          <div class="content">
            <div class="topgrid">
              <div class="card">
                <div class="card-title">Customer queue</div>
                <div class="queue">
                  <div class="queue-badge">${queueNo}</div>
                  <div class="queue-text">
                    <strong>${customer || 'Unknown customer'}</strong>
                    <span>${device || 'Unknown device'} • ${status.label}</span>
                    <div style="margin-top:8px;color:#64748b;font-size:12px;">Received: ${receivedDate}</div>
                  </div>
                </div>
              </div>
              <div class="card">
                ...
                <div class="status-row">
                  ...
                  <span class="pill dark">Repair Ticket</span>
                </div>
              </div>
            </div>

            <div class="card" style="background:#fff;">
              <div class="card-title">Repair information</div>
              <div class="grid">
                <div class="field"><span class="label">Customer</span><div class="value">${customer || '-'}</div></div>
                <div class="field"><span class="label">Phone</span><div class="value">${phone || '-'}</div></div>
                <div class="field"><span class="label">Device</span><div class="value">${device || '-'}</div></div>
                <div class="field"><span class="label">Parts / Service</span><div class="value">${parts || '-'}</div></div>
                <div class="field"><span class="label">Status</span><div class="value">${status.label}</div></div>
                <div class="field"><span class="label">Received Date</span><div class="value">${receivedDate}</div></div>
              </div>

              <div class="section-title">Financial summary</div>
              <div class="summary">
                <div class="sum"><span class="k">Total</span><span class="v">${total}</span></div>
                <div class="sum"><span class="k">Paid</span><span class="v">${paid}</span></div>
                <div class="sum"><span class="k">Balance</span><span class="v">${balance}</span></div>
              </div>

              <div class="section-title">Notes</div>
              <div class="field"><div class="value" style="font-weight:500;">${notes || 'No notes provided.'}</div></div>
            </div>

            <div class="footer">
              <div>Shop Phone: 0617125558</div>
              <div>Working Hours: Sat–Thu • 8:00 AM – 8:00 PM</div>
            </div>
          </div>
        </div>
...
function openRepairDeleteModal(repair, mode = "soft") {
  ...
  showRepairModal("repairDeleteConfirmModal");
}
...
async function handleActionClick(event) {
  ...
  if (action === "edit") {
    ...
    const submit = getEl("repairSubmitBtn");
    if (submit) submit.innerHTML = '<i class="bi bi-check2-circle me-1"></i> Update Repair';
    showToast("Repair loaded into form", "info", "Repair");
    showRepairModal("newRepairModal");
  ...
  if (action === "tracking") {
    ...
    showRepairModal("repairTrackingModal");
    return;
  }
  if (action === "history") {
    await openRepairCustomerHistory(repair);
...
function initFilters() {
  ...
  addTypeBtn?.addEventListener("click", () => {
    renderRepairTypeManager();
    showRepairModal("repairTypeManagerModal");
  });
  ...
  [search, status, date, exactDate, rows].forEach((el) => {
    if (!el) return;
...
function initForm() {
  ...
  ["repairPrice", "repairDiscount", "repairPaidAmount"].forEach((id) => {
    const el = getEl(id);
