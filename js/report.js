...
function updateReportNotificationBadge({ products = [], invoices = [], repairs = [] } = {}) {
  ...
  const lowStockItems = activeProducts
    .filter((item) => safeNumber(item?.quantity) <= 5)
  ...
  const lowStock = activeProducts.filter((item) => safeNumber(item?.quantity) <= 5).length;
...
function findSectionCard(titleText) {
  ...
  const heading = headings.find((el) => normalizeText(el.textContent) === normalizeText(titleText));
...
function renderBestCustomersTable(invoices, repairs, period) {
  const card = findSectionCard("Best Customers");
  if (!card) return;
  const tbody = card.querySelector("tbody");
  if (!tbody) return;
  const rows = buildBestCustomerRows(invoices, repairs);
  if (!rows.length) {
    tbody.innerHTML = `<tr><td colspan="8" class="text-center text-muted py-4">No customer activity in this period.</td></tr>`;
    return;
  }
  tbody.innerHTML = rows.map((row) => `
    <tr>
...
async function loadReportData() {
  const [products, invoices, repairs, expenses] = await Promise.all([
    getProducts(),
    getInvoices(),
    getRepairs(),
    getExpenses()
...
function getReportSnapshot() {
  ...
  const bestCustomerRows = buildBestCustomerRows(invoices, repairs);
  ...
  return { period, bucketRange, invoices, repairs, expenses, products, soldRows, customerHistoryRows, warrantyRows, bestCustomerRows, largestRemainingRows, repairHistoryRows };
}
...
function buildBestCustomerRows(invoices = [], repairs = []) {
  const rows = new Map();
  const upsert = (record, kind) => {
    const key = getCustomerHistoryKey(record);
    if (!key) return;
  ...
  repairs.forEach((repair) => upsert(repair, "repair"));
  return [...rows.values()]
    .sort((a, b) => b.totalAll - a.totalAll || b.lastActivity - a.lastActivity)
    .slice(0, 8)
...
function getReportExportSections(snapshot = getReportSnapshot()) {
  ...
  if (category === "all") {
    ...
    sections.push({ title: `Best customers — ${dateLabel}`, data: makeRows(["Customer", "Phone", "Invoices", "Repairs", "Total All", "Total Paid", "Total Remaining", "Last Activity"], buildBestCustomerRows(invoices, repairs)) });
...
function buildReportPrintHtml(snapshot = getReportSnapshot()) {
  ...
  <div class="sheet">
    <div class="hero">
      <div>
        <h1>Waasuge Electronics Report</h1>
    ...
    ${sectionHtml}
    <div class="footer">Generated by Waasuge Electronics & Mobile Repairing</div>
  </div>
  </body></html>`;
}
...
function exportReportPdf() {
  ...
  doc.setFontSize(18);
  doc.text("Waasuge Electronics Report", margin, y);
  y += 18;
