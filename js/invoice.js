...
const SHOP_NAME = "Waasuge Electricity";
const SHOP_PHONE = "0617125558";
const PAYMENT_SHORTCODE = "*712*617125558*";
...
function buildInvoiceCustomerDirectory() {
  const records = [...getActiveRecords(state.invoices), ...getActiveRecords(state.repairs)];
  const map = new Map();
      ...
      <div class="row g-3">
        ...
        <div class="col-12 col-md-5">
          <div class="p-3 rounded-4 border bg-body-tertiary h-100">
            ...
            <div class="d-flex justify-content-between"><span class="text-muted">Balance</span><strong>${formatCurrency(balance)}</strong></div>
            <hr>
        ...
        <tr data-invoice-row="${invoiceId}">
          ...
          <td data-label="Actions" class="invoice-actions-cell text-end">
            <div class="invoice-action-grid">
              <button class="btn btn-sm btn-outline-secondary action-btn" data-action="view" data-id="${invoiceId}" title="View"><i class="bi bi-eye"></i></button>
              <button class="btn btn-sm btn-outline-info action-btn" data-action="history" data-id="${invoiceId}" title="Customer History"><i class="bi bi-clock-history"></i></button>
              <button class="btn btn-sm btn-outline-primary action-btn" data-action="print" data-id="${invoiceId}" title="Print"><i class="bi bi-printer"></i></button>
              <button class="btn btn-sm btn-outline-success action-btn" data-action="whatsapp" data-id="${invoiceId}" title="WhatsApp"><i class="bi bi-whatsapp"></i></button>
              <button class="btn btn-sm btn-outline-info action-btn" data-action="sms" data-id="${invoiceId}" title="SMS"><i class="bi bi-chat-dots"></i></button>
              <button class="btn btn-sm btn-outline-warning action-btn" data-action="edit" data-id="${invoiceId}" title="Edit"><i class="bi bi-pencil-square"></i></button>
              <button class="btn btn-sm btn-outline-${status === "paid" ? "warning" : "success"} action-btn" data-action="toggle-status" data-id="${invoiceId}" title="${status === "paid" ? "Mark Partial" : "Mark Paid"}">
                <i class="bi ${status === "paid" ? "bi-arrow-counterclockwise" : "bi-cash-coin"}"></i>
              </button>
              <button class="btn btn-sm btn-outline-danger action-btn" data-action="delete" data-id="${invoiceId}" title="Delete"><i class="bi bi-trash3"></i></button>
            </div>
          </td>
        </tr>
      `;
    ...
    <div class="cart-item">
      <div class="d-flex justify-content-between align-items-start gap-3">
        <div>
          ...
          <small class="text-muted">Qty: ${safeNumber(item.qty, 1)} × ${formatCurrency(safeNumber(item.price, 0))}</small>
        </div>
...
function setCreateFieldValues(invoice) {
  ...
  if (fields.paidAmount) fields.paidAmount.value = invoice?.paidAmount ?? "";
...
function formatInvoiceHistoryRows(invoices = [], repairs = [], seed = {}) {
...
function renderCustomerHistoryHtml(summary, rows) {
  const rowMarkup = rows.length
    ...
    : `<tr><td colspan="5" class="text-center text-muted py-4">No history found for this customer.</td></tr>`;
...
async function openInvoiceCustomerHistory(invoice) {
  ...
  try {
    const repairsRaw = await getRepairs();
    state.repairs = toArray(repairsRaw);
    const { summary, combined } = formatInvoiceHistoryRows(state.invoices, state.repairs, invoice || {});
...
function printInvoice(invoice) {
  const html = `
    <html>
      ...
      <body>
        <h1>Electronic Shop Invoice</h1>
        <div class="muted">${formatFriendlyDate(invoice.createdAt)}</div>
        <div class="box">
          <div class="row"><strong>Invoice No</strong><span>${invoice.invoiceNumber || invoice.id || "—"}</span></div>
          <div class="row"><strong>Customer</strong><span>${invoice.customerName || "Direct Sale"}</span></div>
          <div class="row"><strong>Phone</strong><span>${invoice.customerPhone || "—"}</span></div>
          <div class="row"><strong>Status</strong><span>${capitalize(invoice.paymentStatus)}</span></div>
        </div>
        <table>
          <thead><tr><th>Item</th><th>Qty</th><th>Price</th><th>Total</th></tr></thead>
          <tbody>
            ${(toArray(invoice.items || [])).map((item) => {
              const qty = safeNumber(item.qty, 1);
              const price = safeNumber(item.price ?? item.unitPrice ?? 0);
              return `<tr><td>${item.name || item.productName || "Item"}</td><td>${qty}</td><td>${formatCurrency(price)}</td><td>${formatCurrency(qty * price)}</td></tr>`;
            }).join("")}
          </tbody>
        </table>
        <div class="box">
          <div class="row"><span>Subtotal</span><strong>${formatCurrency(invoice.subtotal ?? 0)}</strong></div>
          <div class="row"><span>Discount</span><strong>${formatCurrency(invoice.discount ?? 0)}</strong></div>
          <div class="row"><span>Paid</span><strong>${formatCurrency(invoice.paidAmount ?? 0)}</strong></div>
          <div class="row total"><span>Final Total</span><span>${formatCurrency(invoice.finalTotal ?? 0)}</span></div>
          <div class="row"><span>Remaining</span><strong>${formatCurrency(invoice.balance ?? 0)}</strong></div>
        </div>
...
async function saveInvoiceFromModal(button = null) {
  await saveInvoiceWithGuard(async () => {
    ...
    const items = getCartItemsForInvoice();
    const totals = computeInvoiceTotals(items, modalFields.discount?.value, modalFields.paidAmount?.value);
    const invoice = {
      customerName: modalFields.customerName?.value.trim() || "",
      customerPhone: modalFields.customerPhone?.value.trim() || "",
      invoiceType: modalFields.invoiceType?.value || "Invoice",
      paymentStatus: normalizeStatus(modalFields.paymentStatus?.value || "Unpaid"),
      discount: totals.discount,
      paidAmount: totals.paidAmount,
      balance: totals.remaining,
    ...
    if (safeNumber(totals.paidAmount) > safeNumber(totals.finalTotal)) {
      showToast("Paid amount cannot be bigger than total amount.", "warning", "Invoice");
      return;
    }

    updateInvoiceSaveButton(button || modalFields.saveButton, true, state.editingId ? "Updating..." : (invoice.invoiceType === "Direct Sale" ? "Selling..." : "Saving..."));
    try {
      ...
      showToast(
        state.editingId
          ? `Invoice updated successfully for ${invoice.customerName}.`
...
async function handleInvoiceAction(action, id) {
  ...
  try {
    ...
    if (action === "edit") {
      state.editingId = id;
      setCreateFieldValues(invoice);
      const modal = getModalFields()?.modal;
      const title = document.getElementById("invoiceModalTitle");
      const saveBtn = document.getElementById("saveInvoiceBtn");
      if (title) title.textContent = "Update Invoice";
      if (saveBtn) saveBtn.innerHTML = '<i class="bi bi-check2-circle me-1"></i> Update Invoice';
      window.bootstrap?.Modal.getOrCreateInstance(modal)?.show();
      showToast("Invoice loaded into the form for editing.", "warning", "Edit");
      return;
    }
...
function bindInvoiceFormRealtime() {
  ...
  [fields.discount, fields.paidAmount, fields.invoiceType, fields.paymentStatus].forEach((el) => {
    if (!el) return;
